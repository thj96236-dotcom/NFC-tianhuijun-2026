...
def show():
    ...
    <div class="app">
      <div class="header">
        <div>
          <h1>NIHSS 自动评分工具 (V12.1 临床强化版)</h1>
      ...
      <div class="card">
        ...
        <div class="item">
            ...
            <select id="s2" onchange="calc()">
                ...
                <option value="1">1 - 部分凝视麻痹 (一眼或两眼有异常，但无强迫性性偏斜)</option>
                <option value="2">2 - 强迫性偏斜或完全凝视麻痹 (不能被眼头动作克服)</option>
            </select>
        </div>
      ...
      <div class="card">
        ...
        <div class="item"><label>4. 面瘫 <span id="v4" class="score-tag score-0">0</span></label><select id="s4" onchange="calc()"><option value="0">0 - 正常</option><option value="1">1 - 轻微 (鼻唇沟变浅/微笑不对称)</option><option value="2">2 - 部分 (下半部面瘫)</option><option value="3">3 - 完全 (上下部均瘫痪)</option></select></div>
        <div class="grid-2">
      ...
      <div class="card">
        ...
        <div class="item">
          ...
          <div id="ataxia-warning" class="hint-text">⚠️ 检测到患侧肢体肌力严重受损 (≥3分)，此项自动记为 0 分（不可评）。</div>
    ...
    function calc() {
        ...
        const isAtaxiaInvalid = m56.some(v => v >= 3);
        ...
        ids.forEach(id => {
            ...
            // 特殊逻辑：如果肌力≥3分，共济失调项强制计0分并禁用显示
            if (id === '7' && isAtaxiaInvalid) {
                val = 0;
